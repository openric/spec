...
| Standalone capture app ([`@openric/capture`](https://github.com/openric/capture)) | ✓ Live at [capture.openric.org](https://capture.openric.org) — pure-browser data-entry client against any OpenRiC-conformant server. Paste a server URL + API key, pick an entity type, submit. No Heratio dependency — mirror of the viewer's decoupling story on the capture side. |
