...
## Follow-up 3 — fix the red "SPARQL smoke tests" CI (service v0.16.3 + v0.16.4)
The workflow `phpunit --group smoke` had been failing `No tests executed!` (exit 1) on every push — two compounding causes: (1) **PHPUnit 12 ignores `@group` docblocks** — `SparqlSmokeTest` needed the `#[Group('smoke')]` attribute (v0.16.3); (2) the committed **`phpunit.xml` was missing `<directory>packages/ahg-ric/tests/Feature</directory>`** (uncommitted since session 1's failed patch), so CI never scanned the smoke test's folder (v0.16.4). Both needed. CI now green (run on c668236). Stats dashboard confirmed live (chart + CSV buttons + token gate; 27 page views accruing).
